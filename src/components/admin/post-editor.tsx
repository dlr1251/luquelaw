...
import { POST_CATEGORIES } from "@/lib/posts/types";
...
export function PostEditor({ post, locale, saveAction, deleteAction }: Props) {
  const [markdown, setMarkdown] = useState(() =>
    post?.sections?.length ? sectionsToMarkdown(post.sections) : defaultArticleMarkdown(),
  );
      ...
      <div className="grid gap-4 sm:grid-cols-2">
        <div className="space-y-2 sm:col-span-2">
          ...
          <Input
            id="slug_key"
            name="slug_key"
            defaultValue={post?.slug_key ?? ""}
            placeholder="digital-nomad-visa-colombia"
            pattern="[a-z0-9-]+"
            required
            className="font-mono"
          />
          <p className="text-xs text-muted-foreground">
            {locale === "es" ? `/es/posts/` : `/posts/`}
        ...
        <div className="space-y-2">
          ...
          <select
            id="category"
            name="category"
            defaultValue={post?.category ?? "Immigration"}
        ...
        <div className="space-y-2">
          ...
          <Input
            id="reading_time"
            name="reading_time"
            defaultValue={post?.reading_time ?? "5 min"}
          />
        </div>
